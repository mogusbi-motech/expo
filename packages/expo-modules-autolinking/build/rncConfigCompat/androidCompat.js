...
exports.parseComponentDescriptorsFromFileAsync = exports.parseComponentDescriptorsAsync = exports.parseLibraryNameAsync = exports.parseNativePackageClassNameFromFileAsync = exports.parseNativePackageClassName = exports.parsePackageNameAsync = exports.resolveDependencyConfigImplAndroidAsync = void 0;
const fast_glob_1 = __importDefault(require("fast-glob"));
const promises_1 = __importDefault(require("fs/promises"));
const path_1 = __importDefault(require("path"));
const utils_1 = require("./utils");
async function resolveDependencyConfigImplAndroidAsync(packageRoot, reactNativeConfig) {
    const platformReactNativeConfig = reactNativeConfig?.dependency?.platforms?.android;
    ...
    const packageName = platformReactNativeConfig?.packageName ||
        (await parsePackageNameAsync(path_1.default.join(androidDir, manifest), path_1.default.join(androidDir, gradle)));
    const nativePackageClassName = await parseNativePackageClassName(packageRoot, androidDir);
    if (!nativePackageClassName) {
        return null;
    }
    const packageJson = require(path_1.default.join(packageRoot, 'package.json'));
    const packageImportPath = platformReactNativeConfig?.packageImportPath ||
        `import ${packageName}.${nativePackageClassName};`;
    const packageInstance = platformReactNativeConfig?.packageInstance || `new ${nativePackageClassName}()`;
    const buildTypes = platformReactNativeConfig?.buildTypes || [];
    const dependencyConfiguration = platformReactNativeConfig?.dependencyConfiguration;
    const libraryName = platformReactNativeConfig?.libraryName ||
        (await parseLibraryNameAsync(androidDir, packageJson));
    const componentDescriptors = platformReactNativeConfig?.componentDescriptors ||
        (await parseComponentDescriptorsAsync(packageRoot, packageJson));
    let cmakeListsPath = platformReactNativeConfig?.cmakeListsPath
        ? path_1.default.join(androidDir, platformReactNativeConfig?.cmakeListsPath)
        : path_1.default.join(androidDir, 'build/generated/source/codegen/jni/CMakeLists.txt');
    const cxxModuleCMakeListsModuleName = platformReactNativeConfig?.cxxModuleCMakeListsModuleName || null;
    const cxxModuleHeaderName = platformReactNativeConfig?.cxxModuleHeaderName || null;
    let cxxModuleCMakeListsPath = platformReactNativeConfig?.cxxModuleCMakeListsPath
        ? path_1.default.join(androidDir, platformReactNativeConfig?.cxxModuleCMakeListsPath)
        : null;
 ...
 */
async function parseNativePackageClassName(packageRoot, androidDir) {
    const srcFiles = await (0, fast_glob_1.default)('**/*Package.{java,kt}', { cwd: androidDir });
    try {
        const className = await Promise.any(srcFiles.map((filePath) => parseNativePackageClassNameFromFileAsync(path_1.default.join(androidDir, filePath))));
        return className;
    }
    catch { }
    ...
    try {
        const className = await Promise.any(allSrcFiles.map((filePath) => parseNativePackageClassNameFromFileAsync(path_1.default.join(androidDir, filePath))));
        return className;
    }
    catch { }
    return null;
}
exports.parseNativePackageClassName = parseNativePackageClassName;
let lazyReactPackageRegex = null;
let lazyTurboReactPackageRegex = null;
async function parseNativePackageClassNameFromFileAsync(filePath) {
...
exports.parseNativePackageClassNameFromFileAsync = parseNativePackageClassNameFromFileAsync;
...
exports.parseComponentDescriptorsFromFileAsync = parseComponentDescriptorsFromFileAsync;
