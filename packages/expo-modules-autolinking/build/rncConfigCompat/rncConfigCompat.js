...
async function findDependencyRootsAsync(projectRoot, searchPaths) {
    const packageJson = require(path_1.default.join(projectRoot, 'package.json'));
    const dependencies = [
...
async function resolveDependencyConfigAsync(platform, name, packageRoot) {
    const reactNativeConfig = await (0, reactNativeConfig_1.loadReactNativeConfigAsync)(packageRoot);
    ...
    if (platform === 'android') {
        platformData = await (0, androidCompat_1.resolveDependencyConfigImplAndroidAsync)(packageRoot, reactNativeConfig);
    }
    else if (platform === 'ios') {
        platformData = await (0, iosCompat_1.resolveDependencyConfigImplIosAsync)(packageRoot, reactNativeConfig);
    }
    if (!platformData) {
        return null;
    }
